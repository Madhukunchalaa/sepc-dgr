...
async function run() {
    try {
        console.log('Dropping and recreating taqa_daily_input with high precision...');

        // We drop the table and recreate it with NUMERIC(24,6) for all numeric fields
        // to handle the large integrator values found in the Excel.
        await pool.query('DROP TABLE IF EXISTS taqa_daily_input CASCADE');

        await pool.query(`
 CREATE TABLE taqa_daily_input (
  ...
  -- CHEM INPUT (mirrors 5 fields in Chem Input sheet)
...
CREATE INDEX IF NOT EXISTS idx_taqa_input_plant_date ON taqa_daily_input(plant_id, entry_date DESC);
        `);

        console.log('✅ Table taqa_daily_input recreated with high precision.');
    } catch (e) {
